...
def timer(svd_func,matrix_filename,kwargs,decomposition_dir, run_nbr=5):
    ...
    if isinstance(matrix_filename,list):
        matrix_filename=matrix_filename[0]
        matrix_name = os.path.split(matrix_filename)[-1].split('.')[0]
        matrix_name = matrix_name.split('_')[-1]
    else:
        matrix_name = os.path.split(matrix_filename)[-1].split('.')[0]
...
def cleaner(matrix_dir,filename_list):
    """
    Delete the all matrix generated by data_splitter.py in matrix_dir as hdf5 files

    Parameters
    ----------
    matrix_dir : str
        the relative path to the directory where the matrix generated by data_splitter.py in matrix_dir as hdf5 files
    Returns
    -------
    None 
        Side effect of deleting the .dat files in matrix_dir
